...
Agents add try/catch blocks around anything that _could_ throw, even when there's no meaningful recovery. The catch block usually logs the error and returns a default value or silently continues. This feels robust but actually hides bugs — the system appears to work while silently producing wrong results.
