...
export const userSchema = createSelectSchema(users, {
  id: (schema) =>
    schema.describe("WorkOS user ID for the account.").meta({ examples: ["user_01HXYZABC123"] }),
  email: (schema) =>
    schema.describe("Primary email address for the user.").meta({ examples: ["ada@example.com"] }),
  name: (schema) => schema.describe("Display name from WorkOS, if available."),
  created_at: (schema) => schema.describe("Timestamp when the user record was created."),
